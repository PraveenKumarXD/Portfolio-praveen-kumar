...
const Masonry = ({
  ...
  // ponytail: compute totalHeight so the container can auto-size
  const { grid, totalHeight } = useMemo(() => {
    ...
    return { grid: g, totalHeight: Math.max(...colHeights, 0) };
  }, [columns, items, width]);
  ...
  return (
    <div ref={containerRef} className="masonry-list" style={{ height: totalHeight || '100%' }}>
      {grid.map((item, idx) => (
        <div
          key={item.id}
          data-key={item.id}
          className="masonry-item"
          onClick={() => onItemClick ? onItemClick(idx) : window.open(item.url, '_blank', 'noopener')}
